...
const AMENITIES = ["Parking", "Laundry", "Gym", "Pool", "Pet Friendly", "Furnished"];
...
const renterFormSchema = z.object({
  ...
  preferences: z.object({
    ...
    }).refine((data) => data.min <= data.max, { // Ensure min <= max
...
export default function RenterSetup({
  ...
  const onSubmit = async (data: RenterFormValues) => {
    // Submission only happens conceptually on the last step,
    // but handleSubmit won't fire unless validation passes anyway.
    // This check is technically redundant if using RHF validation properly for steps,
    // but kept for clarity / defensive programming.
    if (step < 3) {
      console.warn("onSubmit called before final step.");
      return;
    }
      ...
      if (result.error) {
        if (result.details && Array.isArray(result.details)) {
          ...
          result.details.forEach((error) => {
            ...
            if (Array.isArray(error.path) && error.path.length > 0 && error.path.every(p => typeof p === 'string' || typeof p === 'number')) {
              ...
              console.warn("Received server error detail with invalid path:", error);
              form.setError("root.serverError", { // Use a specific root error key
                type: "server",
  ...
  const renderStepTitle = () => {
    switch (step) {
      ...
      case 2: return "Housing Preferences";
      case 3: return "Notifications & Settings";
      default: return "";
    }
  };
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen p-4 md:p-6">
      <div className="max-w-2xl w-full mx-auto"> {/* Removed p-6, added to parent */}
        ...
        <Form {...form}>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8"> {/* Increased space */}
            <AnimatePresence mode="wait" initial={false}>
              <motion.div
                key={step}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input type="tel" {...field} placeholder="e.g., 555-123-4567" />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                          )}
                            ...
                            <FormItem>
                              ...
                              <FormControl>
                                <div className="relative flex items-center">
                                  ...
                                  <Input {...field} placeholder="e.g., Software Engineer" className="pl-10" />
                                </div>
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        render={({ field: { value, onChange, ...fieldProps } }) => ( // Destructure value and onChange
                          <FormItem className="space-y-4">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-sm font-medium text-foreground"> {/* Improved visibility */}
                                ${value.min} - ${value.max}
                              </div>
                            </div>
                            <FormControl>
                              <div className="relative flex items-center">
                                ...
                                <Slider
                                  {...fieldProps} // Spread remaining field props like name, ref, etc.
                                  min={500}
                                  max={5000}
                                  step={100}
                                  value={[value.min, value.max]}
                                  onValueChange={(newValue) =>
                                    onChange({ // Call onChange with the object structure
                                      min: newValue[0],
                                      max: newValue[1],
                                    })
                                  }
                                  className="flex-1 pl-10" // Add padding for icon
                                />
                              </div>
                            </FormControl>
                            <FormMessage />
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="space-y-3"> {/* Adjusted spacing */}
                            <FormLabel>Number of Bedrooms</FormLabel>
                            <FormControl>
                              {/* Use grid-flow-col for better responsiveness on small screens */}
                              <div className="grid grid-cols-3 sm:grid-flow-col sm:auto-cols-max gap-2">
                                {BEDROOM_OPTIONS.map((option) => (
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      const newValue = currentSelection.includes(
                                        option.value,
                                      )
                                      ...
                                      field.onChange(newValue.sort((a, b) => a - b)); // Keep sorted
                                    }}
                                  >
                                    {option.label}
                                  </Button>
                    ...
                    <CardContent className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                              <FormLabel className="flex-1 pr-4">Email Notifications</FormLabel>
                              <FormControl>
                                <Switch
                                  checked={field.value}
                                  onCheckedChange={field.onChange}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                              <FormLabel className="flex-1 pr-4">Push Notifications</FormLabel>
                              <FormControl>
                                <Switch
                                  checked={field.value}
                                  onCheckedChange={field.onChange}
                            ...
                            render={({ field }) => (
                              <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-3">
                                ...
                                <div className="space-y-1 leading-none">
                                  <FormLabel htmlFor="newListings">New Property Listings</FormLabel>
                                  <FormDescription>
                                    Receive alerts when new matching properties are listed.
                                  </FormDescription>
                                </div>
                              </FormItem>
                            )}
                            ...
                            render={({ field }) => (
                              <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-3">
                                ...
                                <div className="space-y-1 leading-none">
                                  ...
                                  <FormDescription>
                                    Get notified about changes to your rental applications.
                                  </FormDescription>
                                </div>
                              </FormItem>
                            )}
            ...
            <div className="flex justify-between mt-8"> {/* Increased margin top */}
              <Button type="button" variant="outline" onClick={handleBack} disabled={form.formState.isSubmitting}>
                {step === 1 ? "Back to Role" : "Back"} {/* More specific label */}
              </Button>
              {step < 3 ? (
                <Button type="button" onClick={handleStepContinue} disabled={form.formState.isSubmitting}>
                  Continue
                </Button>
              ) : (
